 ...
 * Available under MIT license <https://raw.github.com/victor-homyakov/xstats.js/master/LICENSE.txt>
 */
(function(window, document) {
    'use strict';

    /** Detect memory object */
    var memoryNS = window.performance || window.webkitPerformance || window.console;
    memoryNS = memoryNS && memoryNS.memory && memoryNS;
     ...
     * @param {Number,String} value The value recorded.
     */
    function record(mode, value) {
        var data = cache.data[mode],
            percent = min(100, 100 * (value / (mode == 'fps' ? 80 : mode == 'ms' ? 1000 : 128)));

        value = mode == 'mem' ? value.toFixed(2) : round(value);
        data.length = [data.length, data.unshift({ 'value': value, 'percent': percent })][0];

        value = floor(value);
        data.min = min(data.min == null ? value : data.min, value);
        data.max = max(data.max == null ? value : data.max, value);
    }
    ...
    function setBar(me, node, percent) {
        var height = 100,
            ...
            value = percent == null ? height : (base - portion).toFixed(2);
